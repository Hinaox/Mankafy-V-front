...
export class ChatComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (savedContext) {
      ...
      this.http.post('/api/chatbot/load-context', { context: this.messages }).subscribe({
        next: () => console.log('Contexte chargé avec succès'),
        error: (err) => console.error('Erreur lors du chargement du contexte', err)
      });
    }
  }
   async sendMessage() {
    ...
    // Envoyer le message au backend
      this.http.post(this.authService.baseUrl("/chatbot"), { message: this.userInput, userId: '1' }).subscribe(
      (response: any) => {
