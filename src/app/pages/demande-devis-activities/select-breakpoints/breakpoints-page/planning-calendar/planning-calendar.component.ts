...
export class PlanningCalendarComponent implements OnChanges {
  ...
  setPlanningClient(data?: PlanningClient) {
    ...
    const depart = this.planningClient?.dateDepart;
    if (depart) {
      const dateDepart = new Date(depart);
      const endDateDepart = new Date(dateDepart);
      endDateDepart.setHours(8);
      this.changeDate(dateDepart);
      // régler le départ à 7h
      dateDepart?.setHours(7);
      this.setUpEvents();
  ...
  setUpEvents() {
    ...
    try {
      ...
      const dateDepart = new Date(this.planningClient.dateDepart);
      dateDepart.setHours(7);
      const endDateDepart = new Date(dateDepart);
      endDateDepart.setHours(8);
      const departEvent = <CalendarSchedulerEvent>{
        id: '99',
        start: dateDepart,
        end: endDateDepart,
        title: 'Départ Tananarive',
      };
      this.events.push(departEvent);
    } catch (error) {
      console.error(error);
    }
  }
